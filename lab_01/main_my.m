...
% в
mu = mean(X);
% по умолчанию равен 0, что эквивалентно x1 = var (a, 0). Деленние на N-1
s_sqr = var(X); % =std(X)^2
...
f = normpdf(x, mu, sigma); % normal probability distribution function
figure;
...
% е)
F = normcdf(x, mu, sigma); % normal cumulative distribution function
figure;
hold on;
