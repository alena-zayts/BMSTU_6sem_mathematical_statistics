...
fprintf('\nв)\n');
mu = mean(X);
s_sqr = var(X); % эквивалентно var(X, 0) (Деленние на N-1), =std(X)^2
fprintf('mu = %f\n', mu);
fprintf('s_sqr = %f\n', s_sqr);
...
fprintf('\n');
for i = 1:m-1
    fprintf('J%i = [%f; %f)  | ', i, ai_array(i), bi_array(i)); 
end
fprintf('J%i = [%f; %f]\n', m, ai_array(m), bi_array(m)); 
for i = 1:m-1
    fprintf('n%i=%2i                      | ', i, ni_array(i));
end
fprintf('n%i=%i\n', m, ni_array(m));
...
hist = histogram('BinEdges', ai_array,'BinCounts', h_array); %histogram(X, m); 
